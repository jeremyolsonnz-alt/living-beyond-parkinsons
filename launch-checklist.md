...
- Every practice page carries its disclaimers (synthesis note, voice-clone disclosure, anatomy warning, complement-not-replacement) and a working, persisted See/Feel/Both toggle.
...
- [ ] **Contact email** — in `public/contact.html`, replace `[ email address to be added ]` with the real address (as a `mailto:` link). Remove the placeholder note.
...
- [ ] Record the five narrations. The exact scripts to read are auto-generated — run `python3 build/generate.py`, then use `build/out/audio/<id>.txt` (the agnostic wording). Use the ElevenLabs voice clone of Jeremy's voice.
...
- [ ] Keep the voice-clone disclosure visible on each page (already written in).
